    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const participantData = {
          ...
          email: email,
          phone: phone,
          subCommittee: document.getElementById('subCommittee').value,
        ...
        // 3. Email is now sent automatically via Cloud Functions
        // We skip client-side PDF generation here to avoid mobile browser freezes.

        // 4. Show Success Screen
        document.getElementById('form-wrapper').style.display = 'none';
        
        // Populate success screen
        document.getElementById('registration-number').textContent = participantData.registrationNumber;
        document.getElementById('s-name').textContent = `${participantData.lastName} ${participantData.firstName}`;
        document.getElementById('s-email').textContent = participantData.email;
        document.getElementById('s-phone').textContent = participantData.phone;
        document.getElementById('s-subcommittee').textContent = participantData.subCommittee || 'Non renseigné';
        
        document.getElementById('success-screen').classList.add('visible');
        
        // Set up Redownload button
        document.getElementById('btn-download-receipt').onclick = async () => {
          ...
          a.href = url;
          a.download = `recu-GRADAA-2026-${participantData.registrationNumber}.pdf`;
          a.click();
        };
        ...
        document.getElementById('btn-resend-email').onclick = async () => {
          ...
          try {
            await window.GradaEmail.resendFromAdmin(docRef.id);
            alert("Email renvoyé avec succès !");
            btn.textContent = '📧 Renvoyer le reçu par email';
          } catch(err) {
